...
namespace Skymu
{
    internal class MessageTools
    {
        public static class BrushesStatic
        {
            ...
            // Drop shadow color
            public static readonly Brush ShadowGray = new SolidColorBrush(Color.FromRgb(0xE2, 0xE2, 0xE2));
        }
        ...
        private static void ProcessMarkdigBlocks(InlineCollection inlines, MarkdownDocument document, string input)
        {
            ...
            for (int i = 0; i < blocks.Count; i++)
            {
                if (i > 0)
                {
                    int prevBlockEndLine = blocks[i - 1].Span.End > 0
        ...
        private static void ProcessInline(InlineCollection inlines, MarkdigInline node)
        {
            switch (node)
            {
                ...
                case CodeInline code:
                    {
                        inlines.Add(new InlineUIContainer(CreateCodeBlock(code.Content, inline: true))
                        {
