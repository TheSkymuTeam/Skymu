...
SH.PromotionTemplates = {
    closeButton: '<a href="#" class="buttonSmall barAction close"><span class="innerButton">x</span></a>',
    unit300x250A: '<div class="promotion ${onLoad}Promotion std${campaignId}" placement="${onLoad}">{{tmpl "closeButton"}}<div class="body ${onLoad}Body"></div></div>',
    unit650x90A:  '<div class="promotion ${onLoad}Promotion bnr${campaignId}" placement="${onLoad}">{{tmpl "closeButton"}}<div class="body ${onLoad}Body"></div></div>',
    lightboxUnitC:'<div class="promotion ${onLoad}Promotion lbx${campaignId}" placement="${onLoad}"><div class="body ${onLoad}Body"></div></div>'
};
SH.PromotionData = [];
SH.Promotions = function() {
    var baseUrl = 'https://skymu.app/community-ads/';
    var mappings = null;
    ...
    var placementToTemplate = {
        'toaster':  'unit300x250A',
        'leader':   'unit650x90A',
        'lightbox': 'lightboxUnitC'
    };

    var nameToSlug = function(name) {
        return name.toLowerCase().replace(/ /g, '-');
    };

    var buildImageUrl = function(dimension, ad) {
        return baseUrl + dimension + '/' + nameToSlug(ad.Name) + '.' + ad.Format;
    };

    var pickAd = function(placement) {
        var dimension = null;
        for (var dim in dimensionToPlacement) {
            if (dimensionToPlacement[dim] === placement) { dimension = dim; break; }
        }
        if (!dimension || !mappings) return null;

        var eligible = [];
        for (var i = 0; i < mappings.length; i++) {
            var dims = mappings[i].Dimensions;
            for (var j = 0; j < dims.length; j++) {
                if (dims[j] === dimension) { eligible.push(mappings[i]); break; }
            }
        }
    ...
    var showPlacement = function(placement) {
        var ad = pickAd(placement);
        if (!ad) return;
        ...
        var imgUrl  = buildImageUrl(dimension, ad);
        var template = placementToTemplate[placement];

        for (var k in SH.PromotionTemplates) {
            $.template(k, SH.PromotionTemplates[k]);
        }

        var elem = $.tmpl(template, [{
            onLoad:    placement,
            campaignId: 0,
            unitClass: 'communityAd'
        }]);

       var dims = dimension.split('x');
var el = elem.get(0);
...
el.style.cursor = 'pointer';
        ...
        elem.click(function(e) {
            if ($(e.target).hasClass('close') || $(e.target).parents('.close').length > 0) {
                e.preventDefault();
                handleClose($(this));
                return;
            }
        ...
        elem.find('a.close').click(function(e) {
            e.preventDefault();
            handleClose(elem);
        });

        handleShow(elem, placement);
    };

    var handleShow = function(elem, placement) {
        var callbacks = SH.Promotions.promoCallbacks;
        if (callbacks && callbacks[placement]) {
            callbacks[placement].onShow(elem, {});
        }
    };

    var handleClose = function(elem) {
        var placement = elem.attr('placement');
        var callbacks = SH.Promotions.promoCallbacks;
        if (callbacks && callbacks[placement]) {
            callbacks[placement].onClose(elem);
        }
    };
    ...
    $.subscribe('skypeHomeLoaded', function() {
        if (!shown) { shown = true; self.loadAndShow(); }
    });
    $.subscribe('showingListener', function(visible) {
        if (visible && shown) { self.loadAndShow(); }
    });
},
        ...
        loadAndShow: function() {
            ...
            $.getJSON(baseUrl + 'mappings.json', function(data) {
                mappings = data;
                self.showAll();
            });
        },
        ...
        showAll: function() {
    var placements = ['leader', 'toaster'];
    var picked = placements[Math.floor(Math.random() * placements.length)];
    showPlacement(picked);
},
