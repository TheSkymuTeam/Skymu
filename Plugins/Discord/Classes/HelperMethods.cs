...
namespace Discord.Classes
{
    internal static class HelperMethods
    {
        ...
        public static async Task<byte[]> GetCachedAvatarAsync(
            string userId,
            string hash,
            DiscordChannelType channel_type
        )
        {
            string cacheDirName =
                channel_type == DiscordChannelType.Group ? "discord-group-avatar-cache"
            ...
            if (!Directory.Exists(avatar_cache_dir))
                Directory.CreateDirectory(avatar_cache_dir);
            ...
            {
                Debug.WriteLine(
                    "Unable to fetch avatar from URL - GetCachedAvatarAsync(). The URL in question is: "
                        + url
                );
            }
            return data;
        ...
        public static string ReplaceIDWithName(JsonArray idArray, string content)
        {
            ...
            foreach (var array in idArray)
            {
                ...
                string displayName =
                    array["member"]?["nick"]?.GetValue<string>()
                    ?? array["global_name"]?.GetValue<string>()
                    ?? array["username"]?.GetValue<string>()
                    ?? "Unknown";

                content = Regex.Replace(content, $@"<@!?{Regex.Escape(id)}>", $"<@{displayName}>");
            }
            return content;
        }
        ...
        public static async Task<string> ReplaceIDWithNameForTyping(string id, string token)
        {
            ...
            string userData = await Core.api.SendAPI(
                $"/users/{id}/profile",
                HttpMethod.Get,
                token,
                null,
        ...
        public static UserConnectionStatus MapStatus(string statusStr)
        {
            return statusStr.ToLower() switch
            {
                ...
                "invisible" => UserConnectionStatus.Invisible,
        ...
        {
            channelId = null;
            string dictChannelId = Discord.Core.UserIdToChannelId.TryGetValue(
                identifier,
                out string mappedChannelId
            )
                ? mappedChannelId
                : null;
            if (dictChannelId != null)
                channelId = dictChannelId;
            else
        ...
        public static IEnumerable<JsonObject> GetUserChannels(bool orderByRecent)
        {
            ...
            if (orderByRecent)
            {
                channels = channels.OrderByDescending(c =>
